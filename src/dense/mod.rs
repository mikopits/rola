...
mod tests {
    ...
    #[test]
    fn test_zero() {
        let Z = DenseMatrix::zeros(5 as usize, 4 as usize);
    ...
    #[test]
    fn test_identity() {
        let I = DenseMatrix::identity(5 as usize);
    ...
    // FIXME: Have to specify type of A
    #[allow(non_snake_case)]
    #[test]
    fn test_trace() {
        use matrix::Matrix;
        ...
        let mat = vec![vec![1, 6, 0],
                       ...
                       vec![4, 0, 3],
                       vec![0, 5, 0]];
        let A: DenseMatrix<usize, usize> = DenseMatrix::new(&mat).unwrap();
        assert_eq!(A.trace(), 6);

        let mat2 = vec![vec![1, 9, 9, 9],
                        vec![9, 2, 9, 9],
                        vec![9, 9, 3, 9]];
        let B: DenseMatrix<usize, usize> = DenseMatrix::new(&mat2).unwrap();
        assert_eq!(B.trace(), 6);
    }
    ...
    fn test_add() {
        let v = vec![vec![1, 2],
                     vec![3, 4]];
        let A: DenseMatrix<usize, usize> = DenseMatrix::new(&v).unwrap();
